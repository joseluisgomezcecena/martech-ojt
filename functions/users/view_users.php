...
$sel = mysqli_query($connection,"select count(*) as allcount from users WHERE user_active = 1 AND user_name != '{$_SESSION['ojt_user_name']}' ORDER BY user_id ASC");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of record with filtering
$sel = mysqli_query($connection,"select count(*) as allcount from users WHERE user_active = 1 AND user_name != '{$_SESSION['ojt_user_name']}' ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from users WHERE user_active =  1 AND user_name != '{$_SESSION['ojt_user_name']}' ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
